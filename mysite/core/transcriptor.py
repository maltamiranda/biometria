...
class Transcriptor(object):
    def parse(self, llamado):
        ...
        os.chdir("/home/ubuntu/biometriaTrans/biometria/static/media/audios/tmp")
        ...
        logging.info('Creando audio de un canal cmd='+cmd_split)
        os.system(cmd_split)
        ...
        for canal in canales:
            ...
            cmd_silence = ('ffmpeg -i ' + canal + ' -af silencedetect=n=-'+str(nivel)+'dB:d=2.5 -f null - '
            + '2> '+canal_texto)
            ...
            for i in range(0, len(start_diag)):
                ...
                if (lapso) > 10:
                    ...
                    #Determinamos los intervalos de silencios de acuerdo a la potencia de cada canal
                    cmd_silence = ('ffmpeg -i ' + newfile + ' -af silencedetect=n=-'+str(nivel_sgmt)+'dB:d=0.8 -f null - '
                    + '2> '+Sgmt_texto)
                    ...
                    #Obtenemos los bloques de diálogo
                    #Validando que el bloque tenga algun silencio
                    if len(start_bloque) > 0 and len(start) > 0:
        ...
        for index, row in data.sort_values(by='inicio', ascending=True).iterrows():
            if row['dialogo'] == "":
                trans = trans + datetime.fromtimestamp(int(row['inicio'])).strftime("%M:%S") + "|" + row['canal'] + "|****||"
            else:
                trans = trans + datetime.fromtimestamp(int(row['inicio'])).strftime("%M:%S") + "|" + row['canal'] + "|" + row['dialogo'] + "||"
